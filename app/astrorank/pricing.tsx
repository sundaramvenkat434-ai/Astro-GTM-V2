...
interface Plan { amount: string; sub: string; mo: string | null; savings: string | null }
...
function BillingToggle({ cycle, onChange }: { cycle: Cycle; onChange: (c: Cycle) => void }) {
  ...
  return (
    <div className="relative inline-flex bg-slate-100 rounded-[14px] p-1">
      {pill && (
        <motion.div
          animate={{ left: pill.left, width: pill.width }}
          initial={false}
          transition={{ type: "spring", stiffness: 400, damping: 30 }}
          className="absolute top-1 bottom-1 bg-white rounded-[10px] shadow-[0_1px_5px_rgba(15,23,42,0.08)]"
        />
      )}
      {TABS.map((tab, i) => (
        <button
          ...
          className={`relative z-10 px-5 py-1.5 rounded-[10px] text-[13.5px] font-medium transition-colors duration-150 whitespace-nowrap ${
      ...
      <span className="text-[13.5px] text-slate-600 leading-snug flex items-center gap-1.5">
        ...
        {hasTooltip && (
          <Tooltip>
            ...
            <TooltipContent side="right" className="max-w-[220px] p-3 text-[12px] leading-relaxed">
...
export default function PricingSection() {
  ...
  return (
    <TooltipProvider>
      <section ref={sectionRef} id="pricing" className="py-14 bg-[#FAFAFA] border-t border-slate-100">
        <div className="max-w-[1280px] mx-auto px-5 lg:px-10">
          ...
          >
            ...
            <div className="relative bg-white rounded-2xl border border-slate-200 shadow-[0_2px_16px_rgba(15,23,42,0.06)] p-8 flex flex-col">
              <div className="mb-6">
                ...
                <div className="text-[2.5rem] font-extrabold text-slate-900 leading-none tracking-tight mb-1.5">
                  FREE
                </div>
                <p className="text-[13.5px] text-slate-400">Best for trying AstroRank before committing.</p>
              </div>

              <button className="w-full py-2.5 rounded-xl border border-slate-200 text-slate-700 text-[13.5px] font-semibold hover:border-slate-300 hover:bg-slate-50/80 active:scale-[0.99] transition-all duration-150 mb-7">
                Start Free
              </button>
            ...
            <div className="relative bg-white rounded-2xl border-2 border-blue-500 shadow-[0_4px_32px_rgba(59,130,246,0.14)] p-8 flex flex-col overflow-hidden">
              ...
              <div className="mb-6">
                <p className="text-[11px] font-semibold text-blue-500 uppercase tracking-widest mb-2">SEO Lite</p>
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="flex items-end gap-2.5 flex-wrap mb-1.5"
                  >
                    <span className="text-[2.5rem] font-extrabold text-slate-900 leading-none tracking-tight">
                      {plan.amount}
                    </span>
                    <div className="flex flex-col pb-0.5 gap-0.5">
                      <span className="text-[13.5px] text-slate-500">{plan.sub}</span>
                      {plan.mo && <span className="text-[12px] text-slate-400">{plan.mo}</span>}
                    </div>
                    {plan.savings && (
                ...
                <p className="text-[13.5px] text-slate-400">Best for growing your organic presence.</p>
              </div>
              ...
              <ul className="flex flex-col gap-2.5">
                {SEO_FEATURES.map(f => <FeatureRow key={f.text} text={f.text} hasTooltip={f.hasTooltip} />)}
              </ul>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="mt-4 max-w-[800px] mx-auto"
          >
