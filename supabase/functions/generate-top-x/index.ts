...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: settingsRows } = await supabase
      .from("admin_settings")
      .select("key, value")
      .in("key", ["top_x_slug_system_prompt", "top_x_content_system_prompt", "top_x_content_system_prompt_v2", "ai_model_generate_top_x", "ai_request_count_generate_top_x", "ai_provider_top_x_slug_system_prompt", "ai_provider_top_x_content_system_prompt", "ai_provider_top_x_content_system_prompt_v2"]);
...
- All content must be publication-ready`;
    ...
    const provider = getProvider(settings, mode === "slug" ? "top_x_slug_system_prompt" : contentPromptKey);
